...
impl<T: MancalaZobrist> Default for ParMinimaxBuilder<T> {
    /// The default [`ParMinimax`] configuration is the following:
    /// - `optimize_for`: [`Player::One`]
    /// - `max_depth`: `18`
    /// - `max_time`: [`None`]
    /// - `iterative_deepening`: [`true`]
...
impl<T: MancalaZobrist> From<ParMinimax<T>> for ParMinimaxBuilder<T> {
    fn from(value: ParMinimax<T>) -> Self {
        par_from_common(&value)
    }
}
...
impl<T: MancalaZobrist> ParMinimaxBuilder<T> {
    /// Construct a new [`ParMinimaxBuilder`] instance using the default configuration.
    ///
    /// See [`ParMinimaxBuilder::default`] for details.
    #[inline]
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(&self) -> ParMinimax<T> {
        // Initially size the transposition table.
        let t_table_buckets = if self.use_t_table && self.t_table_buckets > 0 {
...
/// Helper function for implementing the [`From`] trait.
fn par_from_common<T: MancalaZobrist>(value: &ParMinimax<T>) -> ParMinimaxBuilder<T> {
    ParMinimaxBuilder {
        optimize_for: value.optimize_for,
        max_depth: value.max_depth,
        max_time: value.max_time,
